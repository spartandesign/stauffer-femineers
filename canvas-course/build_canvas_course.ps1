...
<div style="max-width:850px;margin:0 auto;font-family:Arial,Helvetica,sans-serif;color:#25223a;line-height:1.55;">
  ...
  <div style="margin-top:22px;padding:18px;border-left:6px solid #ef6a74;background:#fff3f4;">
    <p style="margin:0 0 8px;"><strong>Evidence choices:</strong> file upload, text entry, website URL, media recording, or a mentor-recorded talk-through when appropriate. Use the format your mentor directs.</p>
    <p style="margin:0;"><strong>Studio completion window:</strong> intentionally unset in this template. Mentors will schedule the workday, lunch checkpoint, debrief, advisory, or another supervised school-time catch-up in Canvas. It is not take-home homework.</p>
  </div>
...
<div style="padding:20px;border:1px solid #ded8f7;border-radius:14px;margin-bottom:18px;">
  ...
  <ul><li>36 Femineers: 18 Wearables students and 18 Creative Robotics students in nine pairs work through six project phases using five full pull-out workdays, three all-program Wednesday lunch checkpoints, and one invitation-only Project Rescue lunch.</li><li>Full workdays meet in Room 14, 8:00 a.m.–2:41 p.m. September 24 follows Thursday bells (snack 9:25–9:38; lunch 12:42–1:12); later workdays follow Monday bells.</li><li>All-program lunch checkpoints meet in Room 14 from 11:37 a.m.–12:07 p.m. on October 21, January 13, and February 10. December 2 is an invitation-only evidence conference for projects needing a decision before December 7.</li><li>Students must check with teachers, collect assignments and notes, and complete all classwork missed during pull-out workdays by each teacher’s deadline.</li><li>Tri Tansopalucks, Jennifer Frausto, and Stephanie Chavez are the three primary teacher mentors.</li><li>Use your iPad for Canvas, evidence, and BirdBlox. Classroom Windows laptops support MakeCode and micro:bit preparation.</li></ul>
</div>
...
'@;
    Links=@(@{Label='Open My Project';Url="$baseUrl/my-project.html"},@{Label='Explore the complete tutorial library';Url="$baseUrl/tutorials.html"},@{Label='Start Phase 1';Url="$baseUrl/start-here.html"})
  },
  [ordered]@{
    Slug='roadmap'; Title='Program Roadmap and Important Dates'; Eyebrow='Orientation';
    Lede='Students are pulled from regular classes for five Stauffer workdays and are responsible for completing missed classwork. Three all-program lunch checkpoints plus one invitation-only Project Rescue conference maintain progress between workdays.';
    Content=@'
...
<tr><td style="padding:10px;border:1px solid #d9d3ef;">Sept. 24</td><td style="padding:10px;border:1px solid #d9d3ef;">Phase 1: Imagine the Future</td><td style="padding:10px;border:1px solid #d9d3ef;">Explore, brainstorm, propose, approve</td></tr>
...
<tr><td style="padding:10px;border:1px solid #d9d3ef;">Dec. 2</td><td style="padding:10px;border:1px solid #d9d3ef;">Project Rescue lunch · by invitation</td><td style="padding:10px;border:1px solid #d9d3ef;">Evidence conference; choose CONTINUE, CUT SCOPE, REPAIR FIRST, or TECH LEAD and stage the December 7 restart</td></tr>
...
'@;
  ...
  [ordered]@{
    Slug='lunch-checkpoints'; Title='All-Program Lunch Checkpoints and Milestone Treats'; Eyebrow='October 21 · January 13 · February 10';
    Lede='At the three all-program checkpoints, submit first, demonstrate progress in Room 14 during lunch, receive a clear readiness status, and leave with one exact next action.';
    Content=@'
...
<div style="padding:16px;background:#edf9f8;border-radius:12px;margin:0 0 18px;"><strong>December 2 Project Rescue is different:</strong> it is an invitation-only support conference, not an all-program checkpoint. There is no new submission, live test, construction, individual reflection, or milestone reward requirement. Students open existing evidence and leave with CONTINUE, CUT SCOPE, REPAIR FIRST, or TECH LEAD plus one safe December 7 starting action.</div>
...
'@;
  ...
  [ordered]@{
    Slug='phase-1-overview'; Title='Phase 1: Imagine the Future'; Eyebrow='Thursday, September 24, 2026';
...
'@;
  ...
  [ordered]@{
    ...
    Content=@'
<h2 style="color:#168b88;">Creative Robotics — 1st year or 3rd-year choice</h2><p>Stay with your partner at one assigned table with one numbered Hummingbird kit, R-labeled micro:bit, iPad, and three-word device name. Switch Driver/Navigator roles—not hardware—after each challenge.</p><ol><li><strong>Controller + reset:</strong> connect only to the assigned device; save a kit-label photo and safe start/reset note.</li><li><strong>Input:</strong> record far/near/far sensor readings, choose a threshold from the evidence, and name the port.</li><li><strong>Two outputs:</strong> test the prepared LED and secured position servo separately and together; save a readable code screenshot and response note.</li><li><strong>Complete interaction:</strong> demonstrate input → decision → two outputs → reset; save the IF/THEN/AND/OTHERWISE rule plus a code screenshot and short video/photo.</li></ol><p>Run SAFE RESET, stop, disconnect BirdBlox, and power off at every transition. Never load MakeCode onto an R-labeled robotics board.</p>
<h2 style="color:#ef6a74;">Wearables — 2nd year or 3rd-year choice</h2><ol><li><strong>Stamp from the Future:</strong> use ordinary embroidery floss on an inspected wooden blank; save a complete or meaningful in-progress photo and explain what the symbol represents.</li><li><strong>Combined Circuit Lab:</strong> test a mentor-prepared sewable-LED loop, then practice a short unpowered conductive-thread connection on felt; save one clear photo and safety caption.</li><li><strong>micro:bit:</strong> modify the prepared MakeCode starter on a Windows laptop; save a physical test photo or readable screenshot and caption.</li><li><strong>NeoPixel:</strong> run and explain a prepared WS2812B system with a removable mentor-built harness; save one test photo and caption. Code editing is an extension after the core safety pass.</li></ol><p>For every caption, state what you tried, what happened, and what you learned. Ordinary embroidery floss is not conductive and stays separate from Circuit Lab materials.</p>
'@;
    Links=@(@{Label='Creative Robotics pathway';Url="$baseUrl/creative-robotics.html"},@{Label='Meet the Hummingbird';Url="$baseUrl/meet-the-hummingbird.html"},@{Label='Stamp from the Future';Url="$baseUrl/stamp-from-the-future.html"},@{Label='Sewable LED basics';Url="$baseUrl/sewable-led-basics.html"},@{Label='Conductive thread basics';Url="$baseUrl/conductive-thread-basics.html"},@{Label='Meet the micro:bit';Url="$baseUrl/meet-the-microbit.html"},@{Label='Meet programmable pixels';Url="$baseUrl/meet-the-neopixels.html"})
  },
  [ordered]@{
...
<p><strong>Low-paper studio routine:</strong> use one sleeved Reusable Engineering Workday Log per project for the goal, stop condition, three observations, one change, retest, evidence names, and next action. Photograph useful notes before erasing; the log is not graded or homework.</p>
...
'@;
    Links=@(@{Label='Phase 3 workday guide';Url="$baseUrl/phase-3-build.html"},@{Label='Reusable Engineering Workday Log';Url="$baseUrl/output/pdf/Reusable-Engineering-Workday-Log.pdf"},@{Label='Build progress evidence';Url="$baseUrl/build-progress-evidence.html"})
  },
  [ordered]@{
...
'@;
  ...
  [ordered]@{
    ...
    Content=@'
<div style="padding:16px;background:#fff0f1;border-radius:12px;margin:0 0 18px;"><strong>December 2 Project Rescue · invitation only:</strong> open existing evidence; do not retrieve projects or use construction materials. In a three-minute conference, show one last-known-good result, compare expected with observed, narrow the blocker, choose one rescue move, and leave with CONTINUE, CUT SCOPE, REPAIR FIRST, or TECH LEAD. Nothing is assigned for home.</div>
...
<p><strong>Low-paper studio routine:</strong> use the same sleeved Reusable Engineering Workday Log to capture the as-found condition, subsystem, success and stop conditions, first result, one change, fair retest, and January action. Photograph useful notes before erasing; the log is not graded or homework.</p>
...
'@;
    Links=@(@{Label='Project Rescue guide';Url="$baseUrl/lunch-checkpoints.html#december-rescue"},@{Label='Phase 4 workday guide';Url="$baseUrl/phase-4-test.html"},@{Label='Reusable Engineering Workday Log';Url="$baseUrl/output/pdf/Reusable-Engineering-Workday-Log.pdf"},@{Label='Mid-build evidence guide';Url="$baseUrl/mid-build-evidence.html"})
  },
  [ordered]@{
...
<p><strong>Low-paper studio routine:</strong> use one sleeved Reusable Engineering Workday Log per project during Femineers time. Photograph useful notes before erasing; the log is a shared thinking surface, not homework or another graded worksheet.</p>
'@;
...
<p><strong>The Gala Readiness Studio Pack stays with the project.</strong> Students complete it through demonstrations and mentor conversations during Femineers time; it is not a take-home assignment.</p>
'@;
...
'@;
  ...
  [ordered]@{
    ...
    Lede='Preparation, novice-friendly background, exact agendas, kickoff rehearsal, lunch-checkpoint review, student reflection, mentor debrief, and next-phase preparation for the district kickoff and five full workdays.';
    Content=@'
<h2 style="color:#5f3dc4;">Use before each workday</h2><ol><li>Review the preparation list and function-test equipment.</li><li>Open the student hub and pathway guides for the current phase.</li><li>Confirm Canvas assignment, evidence model, and mentor gate.</li><li>Assign stations, roles, repair/inspect areas, storage, and cleanup.</li><li>After dismissal, complete the mentor debrief and prepare the next phase.</li></ol><div style="padding:16px;background:#edf9f8;border-radius:12px;margin:0 0 18px;"><strong>Low-paper studio tools:</strong> use the website Print Center for exact page and copy counts. Sleeve station cards, trackers, and engineering conversation mats for reuse; print the two September 24 duplex studio sheets only for the students or teams who will use them. Femineers complete these tools during program time by drawing, circling, pointing, testing, or explaining. They are not graded worksheets and never become homework. Keep full tutorials and Canvas directions digital.</div><p><strong>Room 14:</strong> W1–W3 each hold six Wearables students; R1–R3 each hold three Robotics pairs. Use the dated mentor start page for the current agenda. Detailed four-station plans are an expanded reference requiring the named adult supervision.</p><h2 style="color:#ef6a74;">Four-activity technology reference · additional trained supervision</h2><p>Use four 25-minute rounds, three 3-minute safety transitions, and a final 5-minute evidence/inventory shutdown. Save evidence locally during the lane; upload to Canvas later.</p><h3 style="color:#ef6a74;">Wearables setup</h3><ul><li>Stage 40 inspected wooden blanks, two BYMORE embroidery-floss kits, short pre-cut ordinary-floss lengths, named snack bags, planning cards, tested compatible needles/threaders/snips, needle landing trays or pincushions, and a visible opening needle count.</li><li>With 18 Wearables students, 40 blanks leave 22 before inspection losses. Prepare paper-grid backups as needed. Keep embroidery floss separate from conductive thread and powered Circuit Lab materials.</li><li>Run Future Stamp; a prepared LED polarity/loop test plus short unpowered conductive-thread practice; a pre-opened micro:bit starter; and a prepared NeoPixel system. Code editing is an extension only after the core NeoPixel pass.</li><li>Count needles and verify every powered system is off before releasing movement. Four simultaneous stations require four trained leads plus a technical float when possible; reduce capacity if staffing or tested materials are insufficient.</li></ul><h3 style="color:#168b88;">Robotics setup</h3><ol><li>Pair each team of two with one numbered Hummingbird kit, its R micro:bit, charged iPad, full device name/initials, and one table for the entire block. Students switch Driver/Navigator roles; kits do not move.</li><li>Function-test firmware, the correct BirdBlox connection, device identity, safe reset, and one identical prewired capability board per team: distance sensor in Sensor 1, LED in LED 1, and secured position servo in Servo 1.</li><li>Prepare R1 Connect Check, R2 Read Input, R3 Test Outputs, R4 Complete Interaction, SAFE RESET, port/startup cards, a four-box evidence sheet, known-good swaps, and photo/video fallbacks.</li><li>Run four sequential challenges: controller + safe start/reset; three sensor readings + evidence-based threshold; LED + secured position servo; complete IF/ELSE interaction with one input, two outputs, and reset.</li><li>At every transition: SAFE RESET, stop, disconnect, power off, save, and switch roles. Students do not change wiring, firmware, batteries, terminals, or device assignments.</li><li>Release only fully rehearsed sets. Capacity equals the number of complete, reliable kit/iPad/capability-board sets with adult support. Stagger or use the prepared visual tour for any unavailable set.</li></ol><h2 style="color:#ef6a74;">October 13 Glow Up Your Badge setup</h2><ol><li><strong>Learn the four essentials:</strong> the battery supplies energy; the light uses energy; a complete approved path lights while an open path does not; and polarity/orientation must match when the chosen part requires it. A possible short, damaged battery, warmth, or odor means stop and disconnect power.</li><li><strong>Document one exact candidate kit:</strong> record the light/module, power source and voltage, current limiting, orientation marks, no-solder connection, badge base, attachment, adhesive, student steps, mentor-preassembled steps, and safe-off state. Do not substitute the ordered WS2812B strips for a simple light circuit.</li><li><strong>Build one untouched-kit exemplar:</strong> follow the draft student card exactly, add power last, photograph and number every step, and record total time, help needed, wrong turns, and failure points.</li><li><strong>Prove repeatability:</strong> build 3–5 more samples, complete at least ten on/off or open/closed cycles, repeat attachment/removal, and time a full seat reset.</li><li><strong>Run novice trials:</strong> have people unfamiliar with the badge follow only the card. Revise, mark, or safely preassemble until construction normally takes 10–12 minutes, 90–95% work through ordinary quick help, and mentors coach rather than build.</li><li><strong>Prepare examples and diagnosis tools:</strong> standard, personalized, alternate, and open-circuit examples; a master kit; known-good battery/light/insert; one safe prepared open fault; and a labeled inspect bin. Never prepare a live short example.</li><li><strong>Train every mentor:</strong> each mentor builds one badge, explains the circuit in 45 seconds, diagnoses one prepared fault in 30–45 seconds, performs a known-good swap, completes the safety gate, and resets one seat.</li><li><strong>Produce matching kits:</strong> after the design is frozen, prepare one complete kit per confirmed participant plus 10–15% known-good electronic spares. Quarantine damaged, mismatched, or questionable parts.</li><li><strong>Rehearse the whole rotation:</strong> assign lead/timekeeper, placement/build support, connection/test support, troubleshooting/swap, and reset ownership; then run the exact 0:00–20:00 website plan with actual bins and timer.</li><li><strong>Keep Canvas later:</strong> complete the photo reflection after the event or during advisory, not during the 20-minute workshop.</li></ol><h2 style="color:#168b88;">Use before each lunch checkpoint</h2><ol><li>Review Canvas submissions in advance.</li><li>Assign green, yellow, or red status.</li><li>Prepare one named revision for yellow projects.</li><li>Prioritize red projects for mentor conferences.</li><li>Prepare ingredient-labeled treats and nonfood choices.</li></ol>
<h2 style="color:#5f3dc4;">Later workday rehearsal and release gates</h2>
<h3 style="color:#ef6a74;">November 16 · prototype before permanent work</h3><p>Mentors first build and test one actual four-light Wearables sample plus one complete Hummingbird input → decision → two outputs → reset example. Students run three comparable trials, change one variable, and receive CLEARED, TARGETED REVISION, or INSPECT/HOLD before permanent construction. Keep power, needles, wet paint, motion, repair access, evidence, and final counts under separate named supervision.</p>
<h3 style="color:#ef6a74;">December 2 · Project Rescue lunch</h3><p>Invite only projects whose blocker could consume the December 7 start. Open existing evidence; projects and tools remain stored. Use the five-question conference: working proof, expected/observed, smallest blocker, one rescue move, and release status. Record CONTINUE, CUT SCOPE, REPAIR FIRST, or TECH LEAD plus one safe December 7 start sentence. This is not punishment, construction, homework, or a new Canvas submission.</p>
<h3 style="color:#ef6a74;">December 7 · controlled troubleshooting and winter storage</h3><p>Before the workday, mentors rehearse one safe unpowered Wearables connection fault and one non-damaging Robotics threshold/sensor-direction fault. Students photograph the stored condition, test one subsystem, change one variable, repeat the same condition, and complete the storage gate: zero energy, contained parts, backed-up code, supported project, visible condition, and a verb-first January action. Do not cut or install the final WS2812B strip.</p>
<h3 style="color:#168b88;">January 25 · restore, integrate in layers, test, redesign</h3><p>Mentors restore and prove one known-good example in each pathway before release. Use READY / REPAIR FIRST / MENTOR HOLD lanes. Protect the last passing code/hardware baseline, add one proven layer, retest, and return to the last passing state when a new layer causes failure. Stand-test Wearables before wear; power off before any wiring, sewing, servo, or mechanism change. Record one complete-system baseline, one high-impact variable, and a same-condition retest on the reusable log.</p>
<h3 style="color:#168b88;">February 22 · inspection, two trials, recovery, transport</h3><p>Pack and rehearse one mentor example per pathway from closed container through inspection, trial 1, operator switch/trial 2, safe failure response, 45-second story, and pack-down. Construction ends before inspection. A repair affecting the tested system restarts the two-trial count. Use GALA READY, NAMED REPAIR, or MODIFIED DEMONSTRATION; never force an unsafe live feature.</p>
<div style="padding:16px;background:#edf9f8;border-radius:12px;margin:0 0 18px;"><strong>Exact low-paper use:</strong> November 16, December 7, and January 25 use one sleeved Engineering Workday Log per project. December 2 uses page 2 of the shared mentor lunch tracker only. February 22 uses the pathway inspection plus two-trial Gala pages; project-card and transport pages print only when needed. All remain studio tools, not grades or homework.</div>
...
'@;
    Links=@(@{Label='Open the dated mentor start page';Url="$baseUrl/mentors.html"},@{Label='Open the low-paper Print Center';Url="$baseUrl/mentor-print-center.html"},@{Label='September 24 technology rotations';Url="$baseUrl/mentor-lesson-plans.html#day1-tech-rotations"},@{Label='October 13 kickoff mentor plan';Url="$baseUrl/mentor-lesson-plans.html#kickoff-badge"},@{Label='November 16 detailed mentor plan';Url="$baseUrl/mentor-lesson-plans.html#nov16-background"},@{Label='December 2 Project Rescue';Url="$baseUrl/lunch-checkpoints.html#december-rescue"},@{Label='December 7 detailed mentor plan';Url="$baseUrl/mentor-lesson-plans.html#dec7-background"},@{Label='January 25 detailed mentor plan';Url="$baseUrl/mentor-lesson-plans.html#jan25-background"},@{Label='February 22 detailed mentor plan';Url="$baseUrl/mentor-lesson-plans.html#feb22-background"},@{Label='Open the roadmap';Url="$baseUrl/program-roadmap.html"})
  },
  [ordered]@{
    ...
    Content=@'
<ol><li>Confirm the five full workdays, October 13 district kickoff, three all-program lunch checkpoints, and invitation-only December 2 Project Rescue lunch.</li><li>Review module dates, names, Room 14 schedule, invitation capacity, and the no-construction Project Rescue boundary.</li><li>Set due/availability dates only to the approved in-program studio or checkpoint windows, including October 19 for the Design Ready package if approved. Arrange any catch-up at school with a mentor rather than assigning it at home.</li><li>Schedule the 0-point Kickoff Reflection after October 13 during class, advisory, or another supervised school-time window; excuse non-attendees and keep it out of the final grade.</li><li>Keep the Glow Up Your Badge module unpublished until a mentor-made working example, exact parts record, photographed step/orientation card, 10–12-minute novice trial, 90–95% normal-success result, repeated safety/wear/reset checks, confirmed headcount, complete kit count, and 10–15% known-good spare plan all pass the prototype gate.</li><li>Complete and function-test the pathway working examples and safe prepared faults before November 16, December 7, January 25, and February 22; reduce live-test capacity when trained supervision or known-good equipment is insufficient.</li><li>Use the eight 10-point evidence checkpoints for completion and evidence tracking—not project polish. If a different district grading policy is required, update every point value and student-facing explanation consistently.</li><li>Review allowed file types and media size limits on student iPads.</li><li>Add district-required policies, accommodations, food-reward guidance, and contact information.</li><li>Test every website button and all nine Canvas evidence items in Student View.</li><li>Publish pages, evidence items, and modules only after the audit.</li><li>Publish the course when enrollment and communication are ready.</li></ol>
'@;
...
<p><strong>Wearable:</strong> one individual proposal covers both the LED denim shirt and programmable bucket hat. In that same proposal or Canvas submission, include: a complete or meaningful in-progress Future Stamp photo; one combined LED/conductive-thread Circuit Lab photo; one micro:bit test photo or screenshot; one NeoPixel test photo; and short captions stating what was tried, what happened, and what was learned. The stamp caption also explains the symbol and how it may inform the proposal. Stamp completion and craftsmanship are not the mentor-approval gate.</p><p><strong>Robotics:</strong> submit one shared team packet containing the connection/reset photo and note; three sensor readings, port, and evidence-based threshold; a readable two-output code screenshot plus response photo/note; and the complete interaction rule, code screenshot, and short video/photo showing input → decision → two outputs → reset. Add one individual contribution/learning reflection from each partner.</p>
'@
  },
  [ordered]@{
...
'@
  ...
  [ordered]@{
    ...
    Content=@'
<p><strong>Capture while you work:</strong> use the photos, video, code, and photographed reusable-log notes created during the studio. This checkpoint organizes engineering evidence students already demonstrated; it is not extra homework or a separate written report.</p>
...
'@
  ...
  [ordered]@{
    ...
    Content=@'
<p><strong>Capture while you work:</strong> use the inspection, reliability, backup, project-card, and transport evidence completed with mentors during the studio. The packet stays with the project and is not extra homework or a separate report.</p>
...
'@
  ...
  [ordered]@{
    ...
    Content=@'
<p><strong>Build this during the final studio/debrief window:</strong> choose evidence with a mentor and respond in writing, audio/video, or a mentor-recorded conversation. No take-home essay or new project work is required.</p>
...
$modules = @(
  ...
  [ordered]@{Slug='phase-1';Title='Phase 1 — Imagine the Future — September 24';State='active';Items=@(
  ...
  [ordered]@{Slug='phase-4';Title='Phase 4 — December 2 Project Rescue + December 7 Build & Test';State='active';Items=@(
...
<p><strong>Workday location:</strong> Room 14, 8:00 a.m.–2:41 p.m.; Thursday bells on September 24 and Monday bells on later workdays.</p>
